...
# Paper Lifecycle Health Command

## Purpose

`apex paper lifecycle-health` evaluates the latest successful scheduled paper-pipeline audit containing lifecycle analytics. It produces a deterministic operational health assessment for forward-paper review.

This command does not authorize testnet, funded, production, or real-money execution.
...
Persist deterministic JSON evidence with:
...
Existing report paths are rejected unless `--force-report` is supplied.

## Policy options

The command exposes explicit thresholds for:

- minimum terminal trades;
...
- minimum average realized R multiple;
- minimum realized net PnL;
- whether realized performance is required.

The exact selected policy is serialized into the resulting artifact.

## Evidence integrity

Schema-version-2 reports preserve:

- source run ID and market type;
...
- evaluated health result and analytics snapshot;
- SHA-256 of the complete report payload.

Absolute machine-specific paths are excluded from report identity. Identical evidence and policy therefore produce the same artifact across machines.

Every report contains `execution_authorized: false`.
