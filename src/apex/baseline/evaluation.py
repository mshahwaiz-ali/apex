...
def baseline_report_to_payload(report: BaselineEvaluationReport) -> dict[str, object]:
    """Serialize a baseline evaluation report."""
...
def _assess_strategy(
    ...
    expectancy = _weighted_expectancy(profiles)
    profit_factor = _weighted_profit_factor(profiles)
...
def _weighted_expectancy(profiles: Sequence[HistoricalEdgeProfile]) -> float:
    total = sum(profile.sample_size for profile in profiles)
    return sum(profile.expectancy * profile.sample_size for profile in profiles) / total
