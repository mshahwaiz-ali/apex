...
def detect_swings(
    ...
    *,
    ...
    return tuple(
        sorted(
            results,
            key=lambda item: (item.index, item.kind.value),
        )
    )
...
def _is_extreme(
    ...
    *,
    ...
    if policy is ComparisonPolicy.STRICT:
        if is_high:
            return all(candidate > value for value in neighbours)
        return all(candidate < value for value in neighbours)

    if is_high:
        if not all(candidate >= value for value in neighbours):
            return False
    elif not all(candidate <= value for value in neighbours):
        return False
