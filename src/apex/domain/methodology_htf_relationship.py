...
class TradeDirectionLike(Protocol):
    """Minimal direction contract required by HTF relationship classification."""
...
class HtfRelationshipInput:
    trade_direction: TradeDirectionLike
...
def _direction_matches_bias(
    direction: TradeDirectionLike,
    bias: StructuralBias,
) -> bool:
    return (direction.value == "long" and bias is StructuralBias.BULLISH) or (
        direction.value == "short" and bias is StructuralBias.BEARISH
    )
...
def _direction_opposes_bias(
    ...
    return (direction.value == "long" and bias is StructuralBias.BEARISH) or (
        direction.value == "short" and bias is StructuralBias.BULLISH
    )
...
def classify_htf_relationship(
    ...
    if _direction_matches_bias(value.trade_direction, value.structural_bias):
        return HtfRelationshipAssessment(
    ...
    if _direction_opposes_bias(value.trade_direction, value.structural_bias):
