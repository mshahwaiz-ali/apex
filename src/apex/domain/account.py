...
def evaluate_account_policy(
    ...
    if policy.type is AccountPolicyType.FUNDED:
        provider_matches = (
            policy.provider_name is None
            or state.active_provider_name is None
            or policy.provider_name.strip().casefold()
            == state.active_provider_name.strip().casefold()
        )
        phase_matches = (
            policy.challenge_phase is None
            or state.active_challenge_phase is None
            or policy.challenge_phase.strip().casefold()
            == state.active_challenge_phase.strip().casefold()
        )
        preset_matches = (
            policy.provider_preset_sha256 is None
            or state.active_provider_preset_sha256 is None
            or policy.provider_preset_sha256 == state.active_provider_preset_sha256
        )
