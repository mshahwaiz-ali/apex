...
def _analytics_int(payload: dict[str, Any], key: str) -> int:
    value = payload.get(key)
    return value if isinstance(value, int) and not isinstance(value, bool) else 0


def _analytics_float_text(payload: dict[str, Any], key: str) -> str:
    value = payload.get(key)
...
def _emit_pipeline(
    ...
    *,
    ...
    analytics = dict(result.lifecycle_analytics or {})
    ...
    typer.echo(
        ...
        f"| waiting={_analytics_int(analytics, 'waiting_for_entry')} "
        f"| entered={_analytics_int(analytics, 'entered_trades')} "
        f"| partial_targets={_analytics_int(analytics, 'partial_target_fills')} "
        f"| target_completions={_analytics_int(analytics, 'full_target_completions')} "
        f"| stopped={_analytics_int(analytics, 'stop_loss_exits')} "
        f"| invalidated={_analytics_int(analytics, 'invalidations')} "
        f"| net_pnl={_analytics_float_text(analytics, 'realized_net_pnl')} "
        f"| average_r={_analytics_float_text(analytics, 'average_realized_r_multiple')} "
