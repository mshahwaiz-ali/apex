...
def register_analysis_commands(app: typer.Typer) -> None:
    ...
    def analyze(
        ...
        payload = _without_account_fields(serialize_symbol_analysis(result))
...
def _without_account_fields(payload: dict[str, object]) -> dict[str, object]:
    """Remove legacy account-planning fields from the public analysis payload."""

    return {key: value for key, value in payload.items() if key not in _REMOVED_PUBLIC_FIELDS}
