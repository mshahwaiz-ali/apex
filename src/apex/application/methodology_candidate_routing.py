...
def _candidate_lane_horizon_measurement(
    candidate: TradeCandidate,
    *,
    entry_status: EntryStatus,
) -> CandidateLaneHorizonMeasurement | None:
    try:
    ...
    return measure_candidate_lane_horizon(
        candidate,
...
def _candidate_lane_horizon_assessment(
    ...
    *,
    ...
    measurement = _candidate_lane_horizon_measurement(
        candidate,
        entry_status=entry_status,
    )
    return None if measurement is None else measurement.assessment
...
@dataclass(frozen=True, slots=True)
class CandidateLaneHorizonRoutingAudit:
    candidate_id: str
    strategy: str
    direction: str
    entry_status: str
    initial_measurement: CandidateLaneHorizonMeasurement | None
    final_measurement: CandidateLaneHorizonMeasurement | None
    htf_reclassification_applied: bool
    final_context_lane: OpportunityLane
    final_context_holding_horizon: HoldingHorizon
    context_source: str
...
class MethodologyCandidateRoutingResult:
    ...
    lane_horizon_audits: tuple[CandidateLaneHorizonRoutingAudit, ...] = ()
...
def evaluate_methodology_candidate_routing(
    ...
    *,
    ...
    lane_horizon_audits: list[CandidateLaneHorizonRoutingAudit] = []
    ...
    for strategy in analysis.evaluated_strategies:
        ...
        for candidate in strategy_candidates:
            ...
            initial_lane_horizon_measurement = _candidate_lane_horizon_measurement(
                candidate,
                entry_status=entry_status,
            )
            ...
            final_lane_horizon_measurement = initial_lane_horizon_measurement
            htf_reclassification_applied = False
            ...
            if evaluated_candidate is not candidate:
                ...
                htf_reclassification_applied = True
                final_lane_horizon_measurement = _candidate_lane_horizon_measurement(
                    evaluated_candidate,
            ...
            context_source = (
                "measured_after_htf"
                if htf_reclassification_applied and final_lane_horizon is not None
                else "legacy_fallback_after_htf"
                if htf_reclassification_applied
                else "measured_initial"
                if lane_horizon is not None
                else "legacy_fallback_initial"
            )
            lane_horizon_audits.append(
                CandidateLaneHorizonRoutingAudit(
                    candidate_id=candidate_id,
                    strategy=candidate.strategy.value,
                    direction=candidate.direction.value,
                    entry_status=entry_status.value,
                    initial_measurement=initial_lane_horizon_measurement,
                    final_measurement=final_lane_horizon_measurement,
                    htf_reclassification_applied=htf_reclassification_applied,
                    final_context_lane=context.lane,
                    final_context_holding_horizon=context.holding_horizon,
    ...
    return apply_methodology_candidate_routing(
        ...
        lane_horizon_audits=tuple(lane_horizon_audits),
    )
    ...
    *,
    ...
    if normalized_mode is MethodologyGateMode.SHADOW:
        return MethodologyCandidateRoutingResult(
            ...
            lane_horizon_audits=lane_horizon_audits,
        )
...
def _lane_horizon_measurement_payload(
    measurement: CandidateLaneHorizonMeasurement | None,
) -> dict[str, object]:
    if measurement is None:
        return {
            "available": False,
            "missing_measurements": ["candidate_metadata"],
            "reasons": ["candidate metadata is unavailable"],
            "assessment": None,
        }
    assessment = measurement.assessment
    return {
        "available": measurement.available,
        "missing_measurements": list(measurement.missing_measurements),
...
def _lane_horizon_routing_audit_payload(
    audit: CandidateLaneHorizonRoutingAudit,
) -> dict[str, object]:
    return {
        "candidate_id": audit.candidate_id,
        "strategy": audit.strategy,
        "direction": audit.direction,
        "entry_status": audit.entry_status,
        "initial_measurement": _lane_horizon_measurement_payload(audit.initial_measurement),
        "final_measurement": _lane_horizon_measurement_payload(audit.final_measurement),
        "htf_reclassification_applied": audit.htf_reclassification_applied,
        "final_context_lane": audit.final_context_lane.value,
        "final_context_holding_horizon": audit.final_context_holding_horizon.value,
...
def methodology_candidate_routing_payload(
    ...
    return {
        ...
        "lane_horizon_audits": [
            _lane_horizon_routing_audit_payload(item) for item in result.lane_horizon_audits
        ],
