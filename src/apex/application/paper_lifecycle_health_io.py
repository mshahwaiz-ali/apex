...
PAPER_LIFECYCLE_HEALTH_ARTIFACT_SCHEMA_VERSION = 2
...
def load_latest_paper_lifecycle_health(
    ...
    *,
    ...
    latest_record_sha256: str | None = None
    latest_analytics_sha256: str | None = None
    for line_number, raw_line in enumerate(raw_bytes.decode("utf-8").splitlines(), start=1):
        ...
        latest_record_sha256 = _hash_payload(payload)
        latest_analytics_sha256 = _hash_json_value(analytics)

    if (
        latest is None
        or latest_line_number is None
        or latest_record_sha256 is None
        or latest_analytics_sha256 is None
    ):
        raise ValueError(
    ...
    return PaperLifecycleHealthAudit(
        ...
        log_name=path.name,
        source_line_number=latest_line_number,
        source_record_sha256=latest_record_sha256,
        source_log_sha256=source_log_sha256,
        analytics_sha256=latest_analytics_sha256,
        analytics=analytics,
...
def _hash_payload(payload: dict[str, Any]) -> str:
    return _hash_json_value(payload)
