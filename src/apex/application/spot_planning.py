...
def build_spot_plan(
    ...
    *,
    ...
    prices = tuple(sorted(candidate_prices, reverse=True))[: len(allocations)]
    entries = tuple(
        SpotEntryLeg(
    ...
    target_count = len(config.exit.default_target_allocations)
    first_target = max(request.resistance_price, average_entry + risk_distance)
    target_prices = tuple(first_target + risk_distance * index for index in range(target_count))
    target_plan = SpotTargetPlan(
        targets=tuple(
            SpotTargetLeg(
