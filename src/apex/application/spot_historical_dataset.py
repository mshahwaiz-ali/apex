...
def acquire_spot_historical_dataset(
    *,
    ...
    normalized_symbols = tuple(sorted({_exchange_symbol(symbol) for symbol in symbols}))
...
def _canonical_candles(
    ...
    *,
    ...
    values = {
        ...
        if candle.is_closed
        and candle.symbol.upper().replace("/", "") == symbol
...
def _serialize_candle(candle: Candle) -> dict[str, Any]:
    return {
        "symbol": candle.symbol.upper().replace("/", ""),
...
def _exchange_symbol(value: str) -> str:
    return normalize_market_symbol(value).replace("/", "")
