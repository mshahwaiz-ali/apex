...
def run_chronological_pipeline_backtest(
    ...
    replay_candles = request.candles_by_timeframe[request.replay_timeframe]
    ...
    for decision_index in range(request.candle_limit, len(replay_candles)):
        decision_time = replay_candles[decision_index - 1].close_time
        ...
        if analysis.assessment.setup is None:
            ...
            continue
        future = tuple(candle for candle in replay_candles if candle.open_time >= decision_time)
        if not future:
            skipped_count += 1
            continue
...
class _HistoricalPrefixProvider:
    ...
    def fetch_candles(self, symbol: str, timeframe: str, limit: int = 100) -> list[Candle]:
        ...
        available = tuple(
            ...
            if candle.symbol == symbol and candle.close_time <= self._decision_time
        )
        return list(available[-limit:])

    def fetch_ticker(self, symbol: str) -> TickerSnapshot:
        raise NotImplementedError("historical prefix provider does not expose live ticker data")
