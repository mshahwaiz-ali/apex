...
def score_dimensions(item: ScoredCandidate) -> CandidateScoreDimensions:
    """Group normalized metrics into the four Stage 3 ranking dimensions."""
    ...
    return CandidateScoreDimensions(
        ...
        trade_quality_score=round(metrics["target_space_quality"] * 100.0, 6),
    )
...
def unpenalized_rank_score(item: ScoredCandidate) -> float:
    ...
    return round(
        ...
        + dimensions.trade_quality_score * RANK_SCORE_WEIGHTS["trade_quality_score"],
        6,
    )
