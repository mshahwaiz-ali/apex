...
def _higher_timeframe_contradiction(context: StrategyContext, *, bullish: bool) -> bool:
    checker = getattr(context, "higher_timeframe_contradiction", None)
    if callable(checker):
        return bool(checker(bullish=bullish))
    opposed = _BEARISH if bullish else _BULLISH
    return any(
        frame is not context.decision_frame
        and frame.role in _HIGHER_ROLES
        and frame.structure.trend.direction in opposed
...
def _references_are_too_far(context: StrategyContext) -> bool:
    ...
    nearest_distance = min(
        abs(level.representative_price - context.current_price)
        for level in levels
    )
    return nearest_distance > context.atr * _MAX_REFERENCE_DISTANCE_ATR
