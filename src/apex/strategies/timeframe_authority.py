...
def _retest_state(
    ...
    *,
    ...
    alignment = _alignment(frame, direction=direction)
    closed = tuple(candle for candle in frame.recent_candles if candle.is_closed)
    if not closed:
        return False, False, alignment
    latest = closed[-1]
    ...
    return accepted, failed, alignment
...
def resolve_breakout_direction_authority(
    ...
    elif candidate.strategy in _RETEST_STRATEGIES:
        if retest_failed:
            rejection = "5m_retest_failed"
        elif not retest_accepted:
            rejection = "5m_retest_not_accepted"
    elif execution_alignment in {Alignment.OPPOSED, Alignment.STRONGLY_OPPOSED}:
        rejection = "5m_execution_authority_opposed"
