...
from apex.presentation.actionable_plan import hydrate_actionable_setup
...
def _trigger(setup: Mapping[str, object]) -> str:
    ...
    if not trigger:
        ...
        return "No defensible trigger published"
...
def _pre_entry_invalidation(setup: Mapping[str, object]) -> str:
    invalidation = _mapping(_conditional_plan(setup).get("pre_entry_invalidation"))
    if invalidation:
...
def _plan_title(setup: Mapping[str, object], index: int) -> str:
    ...
    trigger_kind = _trigger_kind(setup)
    if str(setup.get("entry_status") or "").upper() == "MISSED_ENTRY":
        return f"SETUP PLAN {index} • RE-ENTRY"
    if trigger_kind == "retest_hold":
...
def _trade_card(
    ...
    *,
    ...
    setup = hydrate_actionable_setup(opportunity, portfolio_cmp=portfolio_cmp)
    ...
    if targets:
        ...
        body.append(
            render_fields(
                tuple(
                    (f"TP{target_index}", _target_line(target, reference))
    ...
    if activation_suppressed:
        ...
        setup_fields.append(
            (
                ...
                or "No executable or future entry condition was defensibly published",
            )
        )
        ...
        if activation_suppressed:
            ...
            explanation_fields.extend(
                (
                    ...
                    ("Confirmation TF", trigger.get("confirmation_timeframe") or "Unavailable"),
    ...
    if opportunities:
        sections.extend(
            _trade_card(
                ...
                explain=explain,
                portfolio_cmp=portfolio.get("cmp"),
            )
