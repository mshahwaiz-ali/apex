...
from apex.presentation import humanize_code, render_bullets, render_fields, render_section, render_title
...
def _ranking_score(item: Mapping[str, object]) -> float:
    setup = _setup(item)
...
def _identity(item: Mapping[str, object]) -> tuple[str, str, str, str]:
    ...
    return (
        ...
        str(setup.get("candidate_id") or item.get("candidate_id") or entry.get("preferred")),
    )
    ...
    return render_section(
        ...
        render_fields(
            (
                ...
                ("Ranking", "Highest confidence first"),
            )
        ),
    )
...
def render_compact_scan(payload: Mapping[str, object], *, explain: bool = False) -> str:
    ...
    sections.append(_summary(payload, trade_count=len(trades), no_trade_count=len(no_trades)))
