...
def render_paper_pipeline(
    ...
    *,
        ...
        render_section(
            ...
            render_fields(
                (
                    ...
                    ("Average realized R", _r_multiple(analytics.get("average_realized_r_multiple"))),
                )
            ),
        ),
    ...
    if output_mode in {OutputMode.VERBOSE, OutputMode.DEBUG}:
        ...
        if failures:
            sections.append(render_section("Provider failures", render_bullets(_failure_lines(failures))))
    if output_mode is OutputMode.DEBUG:
...
def render_paper_status(
    ...
    *,
    ...
    sections = [
        ...
        render_section("Market status", render_bullets(_market_status_lines(markets) or (UNAVAILABLE,))),
        render_section("Next action", _status_next_action(payload, markets)),
    ]
    if output_mode in {OutputMode.VERBOSE, OutputMode.DEBUG}:
        sections.append(render_section("Operational diagnostics", render_bullets(_market_diagnostic_lines(markets) or (UNAVAILABLE,))))
    if output_mode is OutputMode.DEBUG:
        sections.append(render_section("Deterministic payload summary", _debug_fields(payload)))
    return "\n\n".join(sections)
    ...
    *,
    ...
    sections = [
        ...
        render_section(
            ...
            render_fields(
                (
                    ("Total trades", performance.get("total_trades", payload.get("replayed_count", 0))),
        ...
        render_section("Operator guidance", render_bullets(_guidance_lines(trades) or ("No paper trades require action.",))),
    ]
    if output_mode in {OutputMode.VERBOSE, OutputMode.DEBUG}:
        sections.append(render_section("Trade details", render_bullets(_trade_detail_lines(trades) or (UNAVAILABLE,))))
    if output_mode is OutputMode.DEBUG:
        sections.append(render_section("Deterministic payload summary", _debug_fields(payload)))
    return "\n\n".join(sections)
...
def render_paper_trade(
    ...
    *,
    ...
    sections = [
        render_title(f"Paper Trade — {signal.get('symbol') or payload.get('symbol') or UNAVAILABLE}"),
        render_section(
            "Trade outcome",
            render_fields(
                (
                    ("Result", humanize_code(payload.get("result") or payload.get("status") or state)),
                    ("Direction", humanize_code(signal.get("direction") or payload.get("direction"))),
                    ("Lifecycle state", humanize_code(state)),
                    ("Operator action", humanize_code(payload.get("current_action"))),
                    ("Trade identifier", trade.get("trade_id") or payload.get("trade_id") or UNAVAILABLE),
                )
            ),
        ),
        render_section(
            ...
            render_fields(
                (
                    ...
                    ("Margin", format_amount(_first(plan, signal, "required_margin", "margin"))),
                    ("Wallet exposure", format_percentage(_first(plan, signal, "wallet_exposure_pct"))),
                    ("Maximum modeled loss", format_amount(_first(plan, signal, "maximum_modeled_loss", "max_loss"))),
                )
            ),
        ),
        render_section("Next action", payload.get("instruction") or _trade_next_action(state, reasons)),
    ]
    if reasons:
    ...
    if output_mode in {OutputMode.VERBOSE, OutputMode.DEBUG}:
        ...
        sections.append(render_section("Lifecycle events", render_bullets(_event_lines(events) or (UNAVAILABLE,))))
        sections.append(render_section("Fills and exits", render_bullets(_event_lines(fills) or (UNAVAILABLE,))))
    if output_mode is OutputMode.DEBUG:
        sections.append(render_section("Deterministic payload summary", _debug_fields(payload)))
    return "\n\n".join(sections)
...
def _market_status_lines(values: Sequence[object]) -> tuple[str, ...]:
    ...
    for value in values:
        item = _mapping(value)
        lines.append(
            f"{humanize_code(item.get('market_type'))}: ready={_yes_no(item.get('operationally_ready'))}, "
            f"open={item.get('open_trade_count', 0)}, closed={item.get('closed_trade_count', 0)}, "
...
def _market_diagnostic_lines(values: Sequence[object]) -> tuple[str, ...]:
    ...
    for value in values:
        item = _mapping(value)
        lines.append(
            f"{humanize_code(item.get('market_type'))}: cycle_fresh={_yes_no(item.get('scheduler_fresh'))}, "
            f"intake_fresh={_yes_no(item.get('intake_fresh'))}, pipeline_fresh={_yes_no(item.get('pipeline_fresh'))}, "
...
def _guidance_lines(values: Sequence[object]) -> tuple[str, ...]:
    ...
    for value in values:
        item = _mapping(value)
        lines.append(
            f"{item.get('symbol', UNAVAILABLE)} — {humanize_code(item.get('paper_state'))}: "
...
def _trade_detail_lines(values: Sequence[object]) -> tuple[str, ...]:
    ...
    for value in values:
        item = _mapping(value)
        lines.append(
            f"{item.get('symbol', UNAVAILABLE)} | state={humanize_code(item.get('paper_state'))} "
            f"| action={humanize_code(item.get('current_action'))} | id={item.get('trade_id', UNAVAILABLE)}"
        )
    return tuple(lines)
...
def _event_lines(values: Sequence[object]) -> tuple[str, ...]:
    ...
    for value in values:
        ...
        if item:
            ...
            detail = item.get("reason") or item.get("price") or item.get("timestamp") or UNAVAILABLE
...
def _cycle_next_action(cycle: Mapping[str, object], failures: Sequence[object]) -> str:
    if failures:
        return "Review provider failures, then rerun the cycle after market data is healthy."
    if int(cycle.get("advanced_trade_count", 0) or 0) > 0:
        return "Review advanced trades and any new fills, partial exits, invalidations, or completions."
...
def _pipeline_next_action(
    ...
    if int(intake.get("accepted", 0) or 0) > 0 or int(cycle.get("advanced_trade_count", 0) or 0) > 0:
...
def _status_next_action(payload: Mapping[str, object], markets: Sequence[object]) -> str:
    ...
    for value in markets:
        ...
        if int(item.get("consecutive_pipeline_failures", 0) or 0) > 0:
            return "Investigate the latest pipeline failure and restore successful scheduled runs."
...
def _trade_next_action(state: object, reasons: Sequence[object]) -> str:
    ...
    if any(token in normalized for token in ("TARGET", "STOP", "COMPLETED", "CLOSED", "INVALID")):
