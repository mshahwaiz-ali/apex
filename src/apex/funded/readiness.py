...
def evaluate_funded_readiness(
    *,
    ...
    provider_policy_binding: ProviderPolicyBinding | None = None,
    ...
    if account_policy_type is AccountPolicyType.FUNDED:
        if provider_policy_binding is None:
            reasons.append(FundedReadinessReason.PROVIDER_POLICY_BINDING_REQUIRED)
        else:
            if provider_policy_binding.execution_authorized:
                reasons.append(FundedReadinessReason.PROVIDER_POLICY_MISMATCH)
            if not provider_policy_binding.compatible:
                if "PROVIDER_LIMITS_STALE" in provider_policy_binding.compatibility_reasons:
                    reasons.append(FundedReadinessReason.PROVIDER_LIMITS_STALE)
                else:
                    reasons.append(FundedReadinessReason.PROVIDER_POLICY_MISMATCH)
            if (
                provider_policy_binding.provider_name.strip().casefold()
                != provider_limits.provider_name.strip().casefold()
                or provider_policy_binding.verification_date != provider_limits.verified_on
            ):
                reasons.append(FundedReadinessReason.PROVIDER_POLICY_MISMATCH)
    validation_ready = (
