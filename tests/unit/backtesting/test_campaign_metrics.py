...
def test_part2e_recovery_metadata_exposes_identity_sensitivity_and_economics() -> None:
    trade = simulate_trade(
        _signal(),
        (
            _candle(1, low=97.5, high=100.5, close=98.5),
    ...
    assert trade.metadata["recovery_event_id_strict"]
    assert trade.metadata["recovery_event_id"]
    assert trade.metadata["recovery_event_id_loose"]
    assert trade.metadata["recovery_market_episode_id"]
    assert trade.metadata["aggressive_reclaim_target_reached"] is True
    assert trade.metadata["aggressive_reclaim_positive_net"] is True
    assert trade.metadata["aggressive_reclaim_net_r_gate_passed"] is True


def test_part2e_metrics_report_identity_sensitivity_and_speed_quality() -> None:
    first = simulate_trade(
        _signal(),
        (
            _candle(1, low=97.5, high=100.5, close=98.5),
    ...
    metrics = _sweep_reclaim_metrics((first, second))
    aggressive = metrics["aggressive_reclaim"]
    sensitivity = metrics["event_identity_sensitivity"]

    assert aggressive["raw_entry_count"] == 2
    assert aggressive["unique_event_count"] == 1
    assert aggressive["unique_target_reached_count"] == 1
    assert aggressive["unique_positive_net_count"] == 1
    assert aggressive["unique_speed_performance"]["fast"]["event_count"] == 1
    assert sensitivity["current_unique_event_count"] == 1
    assert sensitivity["market_episode_count"] == 1
    assert metrics["representative_selection_basis"] == ("earliest_generated_signal_pre_outcome")


def test_part2e_paired_metrics_compare_same_event_without_production_change() -> None:
    trade = simulate_trade(
        _signal(),
        (
            _candle(1, low=97.5, high=100.5, close=98.5),
    ...
    metrics = _sweep_reclaim_metrics((trade,))
    paired = metrics["paired_entry_comparison"]

    assert trade.metadata["recovery_entry_pair_classification"] == "both_available"
    assert paired["availability_counts"]["both_available"] == 1
    assert paired["both_available_unique_event_count"] == 1
    assert paired["comparable_net_r_count"] == 1
    assert metrics["diagnostic_only"] is True
    assert metrics["production_behavior_changed"] is False


def test_direct_pair_prediction_uses_entry_time_geometry_only() -> None:
    trade = simulate_trade(
        _signal(),
        (
            _candle(1, low=97.5, high=100.5, close=98.5),
    ...
    assert trade.metadata["recovery_pair_available"] is True
    assert trade.metadata["recovery_pair_predicted_mode"] in {
        "prefer_aggressive",
        "prefer_retest",
        "no_clear_preference",
    }
    assert trade.metadata["recovery_pair_realized_winner"] in {
        "aggressive",
        "retest",
        "tie",
    }
    assert isinstance(trade.metadata["recovery_pair_net_r_delta"], float)
    assert trade.metadata["recovery_pair_aggressive_projected_net_r"] >= 0.0
    assert trade.metadata["recovery_pair_retest_projected_net_r"] >= 0.0


def test_direct_pair_metrics_publish_prediction_accuracy_diagnostic_only() -> None:
    trade = simulate_trade(
        _signal(),
        (
            _candle(1, low=97.5, high=100.5, close=98.5),
    ...
    assert paired["comparison_basis"] == "entry_time_geometry_pre_outcome"
