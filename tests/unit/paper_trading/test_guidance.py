...
def test_legacy_fixture_without_lifecycle_events_uses_compatibility_path() -> None:
    trade = _trade(PaperTradeState.ENTERED)

    guidance = derive_paper_trade_guidance(cast(Any, trade))

    assert not hasattr(trade, "lifecycle_events")
    assert guidance.current_action is CurrentAction.HOLD
    assert guidance.active_stop_price == 98.0
    assert guidance.lifecycle_reason is None
